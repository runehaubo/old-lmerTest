...
fillRowRandTable <- function(term, rand.table, elim.num, reduce.random) {
  ...
  if(reduce.random) {
    ...
    if(term[[1]]$chisq.df==2 && elim.num!=0) {
      ...
      rownames(rand.table.upd) <- 
        paste(
          paste(rep(" ", substring.location( 
            names(term), term[[1]]$term[[1]]$gr.part)$first-2), collapse=""), 
          term[[1]]$term[[1]]$gr.part, 
          collapse="")
...
updateRandTable <- function(infoForTerm, rand.table, 
  ...
  if(!is.null(infoForTerm$term)) {   
    rand.table <- 
      fillRowRandTable(infoForTerm, rand.table, elim.num, reduce.random)  
  } else {
    for(i in 1:length(infoForTerm)) {
      iterm <- infoForTerm[i]
      rand.table <- 
        fillRowRandTable(iterm, rand.table, elim.num, reduce.random) 
    }
  }  
  rand.table
}
...
checkCorr <- function(model) {
  ...
  lnST <- length(modelST)
  for(i in 1:lnST) {    
    if(nrow(modelST[[i]]) > 1) corr.intsl <- TRUE
  } 
  return(corr.intsl) 
}
